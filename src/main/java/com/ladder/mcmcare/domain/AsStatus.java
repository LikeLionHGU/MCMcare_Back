...
public enum AsStatus {
    ...
    public boolean canAdminProgressTo(AsStatus next) {
        ...
        // 바로 다음 단계로만 옮길 수 있다.
        //
        // 건너뛰기를 허용하면 상세 화면 타임라인이 모순된다.
        // PICKED_UP → COMPLETED 로 뛰면 중간 단계는 발생한 적이 없으므로
        // "완료" 아래에 "손상부위 진단중 예정" 같은 항목이 붙는다.
        //
        // 이 서비스는 수선 과정을 단계별로 보여주는 것이 핵심이라,
        // 상태와 타임라인이 어긋나면 신뢰를 잃는다.
        return to == from + 1;
    }
